...
- **Personnel management is deliberately out of app scope (added 8/15, Vin review).** Staff identity (adding case managers / nurses as personnel change) belongs to BetterRX identity + the EMR sync in production, not to this app; we do not build user CRUD on a demo with no auth. The **DON is the administrative persona in-app**: approvals, vendor management (view 13), and guardrails (view 14). If judges ask who adds a new nurse: "they already exist in BetterRX identity; the roster syncs."
- **Vendor multi-hospice reality (added 8/15).** A vendor's run list is naturally cross-hospice: the magic link is vendor-scoped, stops are hospice-tagged (`orders.hospice_account`). Demo seeds one hospice and says so; N6 should render the hospice name on every stop so the cross-hospice shape is visible.
