...
>
> This compare screen is the first cost lever. Diego sees each vendor's price and reliability score before he commits, and today that price is invisible until the invoice shows up. These are sample scores, the card says so right on it, and the same formula starts running on real events from day one.
>
...
>
> This receipt is the whole point of the build. Under the standard hospice DME agreement, rental keeps billing until the hospice notifies the vendor. This timestamp is proof of notification down to the minute, captured at two in the morning without waking anyone up, and that's why it's printed so large.
...
## Beat 8, The vendor side is a protocol, not a portal (3:40-4:05)

**Screen: Slide 4** (vendor SMS exchange mock + JSON contract line). Back to slides.

> The whole vendor side is designed around one fact, which is that the baseline vendor never logs into anything. A dispatcher just answers a text message, and the reply flows onto the order timeline. To be straight with you, the live SMS loop isn't wired up in this build. The seam for it is, and the message ladder and reply parser are built and tested behind it. And because that seam is a JSON contract, a dispatcher answers it by text today and a vendor's AI agent can POST the same payload tomorrow.
...
- **"Why would vendors participate?"** (Nathaniel's incentive frame, 8/15): zero friction (no login, no account, no software to install), the reliability score is visible to them and disputable line by line, and the condition loop saves them money: a driver round trip to swap dirty equipment costs the vendor real dollars, so the score pays them to get it right the first time.
