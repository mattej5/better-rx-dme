...
**Amendment 2026-08-14 evening (team call):** SMS is THE channel, for real — Twilio (Nathaniel owns; no-CC trial, ~$0.0087/text), superseding Resend-as-SMS (Resend keeps family notices + fallback). Telegram rejected: requires vendor app install, breaks the zero-adoption principle. GPS demoted to supporting signal — location without route position is meaningless; statuses are Amazon-staged (confirmed → out for delivery → arriving) driven by SMS replies, GPS refines ETA after opt-in. The agent is strictly tool-choice constrained: parse reply → pick from a fixed action list (update status / set ETA / flag at-risk / clarify / escalate). No open conversation, no research. LLM provider swappable behind the seam (Gemini key available). Options considered + pros/cons captured in Nathaniel's brainstorm (manual in-app, email, GPS, SMS, in-app messaging, Telegram). `[team]`
